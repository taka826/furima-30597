class UserItem
  ...
  attr_accessor :postal_code, :area_id, :city, :house_number, :phone, :build_name, :order_id, :token, :user_id, :item_id
 #addressから切り取ったバリデーション
  with_options presence: true do
    validates :postal_code, format: { with: /\d{3}-\d{4}/, message: "Postal code is invalid. Include hyphen(-)"}
    validates :area_id
    validates :city
